"""mv_baseline.py — HAI baseline KHONG debate, chay qua MAD.py.
...
logio.load_rounds() glob "results/logs/**/debate_full_*.jsonl" nen khong bao gio
tron log baseline vao phan tich debate.
...
logger = logging.getLogger("mv_baseline")
...
def run_one(task_benchmark, sample_id: int, question: str, voters: List[tuple],
    ...
    # base_agent._call_ollama truyen MAD_SEED xuong sampler cua Ollama. Cung model
    # + cung prompt + cung seed => cung output, nen neu giu nguyen MAD_SEED thi
    # n_votes phieu cua mot model se GIONG HET nhau va --mv_votes vo nghia. Phieu
    # k=0 giu dung seed goc (nen --mv_votes 1 tai lap y het mot lan chay thuong);
    # k>0 lay seed dan xuat, van xac dinh => van tai lap duoc.
    ...
    try:
        for name, config_path, model_name in voters:
            ...
            for k in range(n_votes):
                ...
                os.environ["MAD_SEED"] = str(vote_seed)
        ...
        os.environ["MAD_SEED"] = str(base_seed)
...
def run_majority_vote_task(task: str, n: int, seed: int, resume: bool, n_votes: int,
                           ...
                           sc_model: Optional[str] = None) -> None:
    ...
    with open(jsonl_path, "a", encoding="utf-8") as jf:
        for i, (sample_id, sample) in enumerate(samples, 1):
            ...
            # self-consistency: giu DUNG mot voter, lay n_votes mau tu chinh no.
            # agent_c van xoay theo sample_id giong debate, de cung sample_id thi
            # ca hai nhanh dung cung model.
            voters = ([all_voters["abc".index(sc_model)]] if sc_model else all_voters)

            t0 = time.time()
            try:
                record = run_one(benchmark, sample_id, sample["question"], voters,
                                 ground_truth=sample.get("answer"), n_votes=n_votes,
                                 mode=mode)
            except Exception as e:
